...
class EntityHelper {
  ...
  def printFilteredTable(table: Seq[AnyRef],filterList:List[String])(implicit out: PrintWriter) =
    out.println(Tabulator.format(getFilteredTable(table,filterList)))

  def printTable(table: Seq[AnyRef])(implicit out: PrintWriter) = out.println(Tabulator.format(getWholeTable(table)))

  def printTableHeader(table: Seq[AnyRef])(implicit out: PrintWriter) = out.println(Tabulator.format(getTableHeaders(table)))
